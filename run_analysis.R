...
all.data.labeled<-all.data.labeled%>%select(matches("(([M,m]ean|[S,s]td))|activity|subject_id"))
...
column.names<-setdiff(names(all.data.labeled),c("subject_id","activity"))
...
tidy.data <-aggregate(all.data.labeled[,column.names], list(Activity=all.data.labeled$activity,Subject=all.data.labeled$subject_id), FUN=mean)
